...
## ملحق Messaging Delta

### REST
- `GET /api/messaging/requests/{request_id}/messages/`
  - كل رسالة تتضمن:
    - `read_by_ids: number[]` (معرفات المستخدمين الذين قرؤوا الرسالة)
- `POST /api/messaging/requests/{request_id}/messages/read/`
  - يعيد:
    - `marked` (عدد الرسائل التي تم تعليمها)
    - `message_ids: number[]` (IDs الرسائل التي تغيّرت حالتها)
...
### فائدة Flutter
- يمكن تعليم الرسائل المقروءة بدقة باستخدام `message_ids` بدل الاعتماد على عدد `marked` فقط.
